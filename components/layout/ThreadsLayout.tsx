...
import { Home, Calendar, Edit, Bell, User, Search } from 'lucide-react';

interface ThreadsLayoutProps {
  children: React.ReactNode;
}

export function ThreadsLayout({ children }: ThreadsLayoutProps) {
  const pathname = usePathname();

  const navItems = [
    { href: '/', icon: Home, label: 'ホーム' },
    { href: '/search', icon: Search, label: '検索' },
    { href: '/composer', icon: Edit, label: '投稿' },
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-gray-200 z-40">
