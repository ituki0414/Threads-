...
予約投稿を指定時間に自動的に投稿するため、外部Cronサービスを使用します。
...
#### 基本設定:
- **Title**: `ThreadStep - 予約投稿の自動実行`
- **URL**: `https://atnoise.jp/api/cron/publish-scheduled`
- **Schedule**:
  - **Execution**: `Every minute` または `Every 5 minutes`
  - 推奨は`Every 5 minutes`（無料枠に優しい）
...
**実行間隔**: 5分ごと
- 理由: API制限を避け、確実に動作する
- 遅延: 最大5分の遅延が発生する可能性あり

**毎分実行したい場合**: Vercel Proプラン($20/月)にアップグレードが必要
